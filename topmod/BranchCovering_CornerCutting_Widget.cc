...
BranchCovering_CornerCutting_Widget::BranchCovering_CornerCutting_Widget(QWidget *parent)
    : QWidget(parent)
{
   QVBoxLayout *layout = new QVBoxLayout(this);

   shrinking_factor_title = new QLabel("<b>Shrink:</b> " + QString::number(parameters.shrinking_factor,'g',3),this);
   ...
   shrinking_factor_slider->setValue(static_cast<int>(parameters.shrinking_factor*SLIDER_MAX));
   connect(shrinking_factor_slider,SIGNAL(valueChanged(int)),this,SLOT(update_shrinking_factor(int)));
   {
   ...
   rotation_factor_title = new QLabel("<b>Rotate:</b> " + QString::number(parameters.rotation*180/PI,'g',3),this);
   ...
   connect(rotation_factor_slider,SIGNAL(valueChanged(int)),this,SLOT(update_rotation_factor(int)));
   {
       QHBoxLayout* row = new QHBoxLayout;
...

   {
       ...
       connect(thickness_box,SIGNAL(valueChanged(double)),this,SLOT(update_thickness(double)));
       {
           QHBoxLayout* row = new QHBoxLayout;
...

   {
       ...
       connect(panel_offset_box,SIGNAL(valueChanged(double)),this,SLOT(update_panel_offset(double)));
       {
           QHBoxLayout* row = new QHBoxLayout;
   ...
   {
       QLabel* panel_alternation_offset_title = new QLabel(tr("<b>Alternation Offset:</b>"),this);
       ...
       connect(panel_alternation_offset_box,SIGNAL(valueChanged(double)),this,SLOT(update_panel_alternation_offset(double)));
       {
           QHBoxLayout* row = new QHBoxLayout;
   ...
   connect(add_backward_btn,SIGNAL(pressed()),this,SLOT(add_backward_symbol()));
   ...
   connect(move_down_symbol_btn,SIGNAL(pressed()),this,SLOT(move_down_symbol()));
   ...
   update_stack();
   {
        ...
        {
            QVBoxLayout* column = new QVBoxLayout;
            column->addWidget(new QLabel(tr("<b>Edge Mapping</b>"),this),0,Qt::AlignLeft);
...
void BranchCovering_CornerCutting_Widget::update_shrinking_factor(const int &value)
{
    parameters.shrinking_factor = (static_cast<double>(value))/SLIDER_MAX;
    shrinking_factor_title->setText("<b>Shrink:</b> " + QString::number(parameters.shrinking_factor,'g',3));
}

void BranchCovering_CornerCutting_Widget::update_rotation_factor(const int &value)
{
    parameters.rotation = (static_cast<double>(value))/SLIDER_MAX;
    rotation_factor_title->setText("<b>Rotate:</b> " + QString::number(parameters.rotation*180,'g',3));
    parameters.rotation *= PI;
}

void BranchCovering_CornerCutting_Widget::update_thickness(const double &value)
{
    parameters.thickness=value;
}

void BranchCovering_CornerCutting_Widget::update_panel_offset(const double &value)
{
    parameters.panel_offset=value;
}

void BranchCovering_CornerCutting_Widget::update_panel_alternation_offset(const double &value)
{
...
{
    int row = mapping_view->currentRow();
    if(row == mapping_view->count() -1)return;
    QListWidgetItem *item = mapping_view->takeItem(row);
    mapping_view->insertItem(row+1,item);
    mapping_view->setCurrentRow(row+1);
    update_stack();
...
void BranchCovering_CornerCutting_Widget::update_stack()
{
    ...
    for (int i = 0; i < mapping_view->count(); ++i) {
        ...
        if(str == SYMBOLS[0])mapping_operations.push_back(HALF_EDGE_FORWARD);
        else if(str == SYMBOLS[1])mapping_operations.push_back(HALF_EDGE_CROSS);
        else /*(str == SYMBOLS[2])*/mapping_operations.push_back(HALF_EDGE_BACKWARD);
    }
