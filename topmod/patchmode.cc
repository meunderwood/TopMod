...
//    parameters.polygonRegularizationIteration=0;
    ...
    //Scale is multiplied by 100 to convert it to a double and get a smoother tick
    ...
    connect(scaleSlider, SIGNAL(valueChanged(int)), this, SLOT(scaleValueChanged(int)));
    ...
    connect(angleSlider, SIGNAL(valueChanged(int)),this, SLOT(angleValueChanged(int)));
    ...
    connect(subDivisionBox, SIGNAL(valueChanged(int)), this, SLOT(subDivisionChanged(int)));
    ...
    connect(sudivisionSlider, SIGNAL(valueChanged(int)), this, SLOT(patchNestingFactorChanged(int)));

    localPatchNestingChecked = new QCheckBox("Locally subdivide nested patches", this);

    renderPatchBoundaries = new QCheckBox("Render patch boundaries", this);
    connect(renderPatchBoundaries, SIGNAL(clicked(bool)), this, SLOT(renderPatchBoundariesChanged(bool)));

    renderDooSabinPolygons = new QCheckBox("Show Doo-Sabin Polygons", this);
    connect(renderDooSabinPolygons, SIGNAL(clicked(bool)), this, SLOT(renderDooSabinPolygonsChanged(bool)));

    renderModifiedDooSabinPolygons = new QCheckBox("Show Modified Doo-Sabin Polygons", this);
    connect(renderModifiedDooSabinPolygons, SIGNAL(clicked(bool)), this, SLOT(renderModifiedDooSabinPolygonsChanged(bool)));



    localPatchNestingChecked->setChecked(false);
    connect(localPatchNestingChecked, SIGNAL(clicked(bool)), this, SLOT(patchNestingMethodChanged(bool)));
    ...
    connect(regularizePolygons, SIGNAL(clicked(bool)), this, SLOT(regularizePolygonsChanged(bool)));
    ...
    connect(polygonRegularizationIterationSlider, SIGNAL(valueChanged(int)), this, SLOT(polygonRegularizationIterationSliderChanged(int)));
    ...
    connect(vertexMovementFactorSlider, SIGNAL(valueChanged(int)), this, SLOT(vertexMovementFactorChanged(int)));
    */

    this->showPatchControlMesh = new QCheckBox("Show Patch Control Mesh", this);
    connect(showPatchControlMesh, SIGNAL(clicked(bool)),this, SLOT(showPatchControlMeshChanged(bool)));
...
void PatchMode::polygonRegularizationIterationSliderChanged(const int &value){
    ...
    QString s = "Iterations: " + QString::number(this->parameters.polygonRegularizationIteration);
...
//    this->vertexMovementFactorStringValue->setText(QString::number(this->parameters.vertex_movement_factor, 'g', 3));
//    this->updated();
//}
...
//    qDebug() << "UPDATED: Angle " << QString::number(this->parameters.angle, 'g', 3)
//             << "; Cos "<< QString::number(this->parameters.cos_angle, 'g', 3)
//             << "; Sin "<< QString::number(this->parameters.sin_angle, 'g', 3)
//             << "; Scale "<< QString::number(this->parameters.scale, 'g', 3)
//             << "; Mode "<< QString::number(this->parameters.rotation_method, 'g', 3)
//             << "; Subdivision " << QString::number(this->parameters.grid_size);
...
void PatchMode::convertTriggered(){
    emit ConvertToTopMode();
}

void PatchMode::saveAsObjTriggered(){
    emit SaveAsOBJ();
}
