...
{
    this->vertexCount = 0;
    this->uv_size = (1<<TMPatch::PatchNestingFactor)*TMPatch::PatchGridDimension;
...
void BiCubicBezierPatchToDLFLData::processBezierPatch(const TMPatch &patch){
    ...
    while (i_index<=this->uv_size) {
        ...
        while (j_index<=this->uv_size) {
            Point3d pnt = patch.point_at(this->UV_values[i_index], this->UV_values[j_index]);
            pnts.push_back(pnt);
    ...
    for (int i = 0; i < gridSize - 1; ++i) {
        for (int j = 0; j < gridSize - 1; ++j) {
            QuadFace *newFace = new QuadFace
                (
                point_to_index.value(grid[i][j]),
...
struct patch_data BiCubicBezierPatchToDLFLData::get_patch_data(){
    ...
    for (int i = 0; i < this->faces.size(); ++i) {
        ...
        if (data.face_indices[i*4] == 0 ||
            data.face_indices[i*4 +1] == 0 ||
...
//this function was developed for debug purpose only. I thought it might be useful and I did not remove it
void BiCubicBezierPatchToDLFLData::toFile(QString address){
    ...
    last = sortedPoints.end();
    while (first != last) {
        out << "v" << " " << (*first)[0] << " " << (*first)[1] << " " << (*first)[2] <<"\n";
        first++;
    }
    out << "# Faces OBJ"<<"\n";
    //write the faces
    for (int i = 0; i < this->faces.size(); ++i) {
        out << "f" << " " << (*(faces[i]))[0] << " " << (*(faces[i]))[1] << " " << (*(faces[i]))[2] << " " << (*(faces[i]))[3] << "\n";
    }
